...
```      
      ...
      use_tasks_queue:bool = False (Optional) # use fastdb.TasksQueue to speed up database operations
```   
...
class Database:
   '''
   fastdb Database Main Class (Do not use this to connect to your database)

   '''
   def __init__(self, db:str, use_tasks_queue:bool=False):
      '''
      Initialize fastdb Database
...
class Collection:
   ...
   def build(self, *db:str, model:str):
      dbs = list(map(lambda e:join(self.collection_path, e), list(db)))
      build_db(*dbs, model=model)

   def destroy(self, db:str, yes:bool=False):
      destroy_db(join(self.collection_path, db), yes)

   def init(self, name:str="db"):
      init(join(self.collection_path, name))
...
def connect(path:str, *, model:str=None, use_tasks_queue:bool=False, make_collection:bool=None, build:bool=False) -> Union[DocumentDatabase, KeyValDatabase, Collection]:
   '''
   `connect()` is used to connect to the database easily

   can be used instead of calling `fastdb.database.DocumentDatabase()` directly
   ...
   if (make_collection is None or make_collection is True or make_collection) and isdir(path):
      ...
      if not model:
         raise ValueError("model parameter is required in build mode")
      build_db(path, model=model)
...
def destroy_db(db:str, yes:bool=False):
   '''
   Delete a database
   
   yes:str=False if True it will skip the verification
   '''
   verification = input("Are your sure? this can't be undone (y/n)> ").strip().lower() if not yes else "y"
   correctAnswer = False
   YESs = ["y", "yes"]
   NOs = ["n", "no"]
